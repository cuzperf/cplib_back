...
class Poly : public T {
 ...
 public:
  ...
  // assume a[0] = 1
  Poly log(int n) const { return (derivation() * inv(n)).integral().modXnR(n); }
  // assume a[0] = 0
  ...
  // compose poly common algorithm F(A(x)) in $O(n^2)$, however Brent-Kung algorithm with $(n \log
  // n)^{1.5}$ may be slower.
  ...
  // compose inverse (assmue a[0] = 0, a[1] \neq 0) based on Langrange: $[x^n]G(x) =
  // \frac{1}{n}[x^{n - 1}](\frac{x}{F(x)})^n$
  Poly composeInv(int n) const {
    ...
    h[0] = {1};
    for (int i = 1; i <= sn; ++i) {
      h[i] = (h[i - 1] * A).modXnR(n - 1);
    }
    std::vector<valT> ans(n), inv(n);
    auto M = valT::mod();
    inv[1] = 1;
    for (int i = 2; i < n; ++i) {
      inv[i] = inv[M % i] * valT(M - M / i);
    }
    Poly now{1};
    for (int i = 0; i < n; i += sn) {
  ...
  std::vector<valT> evals(const std::vector<valT>& x) const {
    ...
    build(0, n, 1);
    std::function<void(int, int, int, const Poly&)> solve = [&](int l, int r, int p,
                                                                const Poly& f) {
      if (r - l == 1) {
        ans[l] = f.at(0);
      } else {
  ...
  static std::vector<valT> sumFraction(std::vector<valT> a, std::vector<valT> b, int N) {
    std::function<std::pair<Poly, Poly>(int, int)> solve = [&](int l,
                                                               int r) -> std::pair<Poly, Poly> {
      if (r - l == 1) {
...
// There will be `PolyNTT`, `PolyFFT`, `PolyFFTDynamic`, `PolyMFT` provided to suit for different
// module $M$.
