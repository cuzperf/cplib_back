...
class SegTree {
  ...
  void pushUp(int p) { tree[p].mn = std::min(tree[tree[p].l].mn, tree[tree[p].r].mn); }
  ...
  void pushDown(int p) {
    if (tree[p].l) {
      updateNode(tree[p].l, tree[p].val);
    }
    if (tree[p].r) {
      updateNode(tree[p].r, tree[p].val);
    }
    tree[p].val = 0;
  }
  ...
  int query(int l, int r, int p) {
    ...
    int ans = 0;
    if (tree[p].l) {
      ans += query(l, m, tree[p].l);
    }
    if (tree[p].r) {
      ans += query(m, r, tree[p].r);
    }
    return ans;
  }
 ...
 public:
  SegTree() { newNode(); }
  void add(int L, int R, int val) { add(L, R, val, -INF, INF, 0); }
  int query() { return query(-INF, INF, 0); }
};
