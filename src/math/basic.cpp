...
int inv(int a, int n) { return a == 1 ? 1 : 1LL * (n - n / a) * inv(n % a, n) % n; }

static uint64_t gcd_(uint64_t a, uint64_t b) {
  if (!a || !b) {
    return a | b;
  }
...
int64_t gcd(int64_t a, int64_t b) { return gcd_(std::abs(a), std::abs(b)); }
