...
class SegmentTreeT {
  int n_;
  std::vector<T> info_;
  void pull(int p) { info_[p] = info_[p << 1] + info_[p << 1 | 1]; }
  void modify(const T& v, int x, int l, int r, int p) {
    if (r - l == 1) {
      info_[p] = v;
    } else {
      int m = (l + r) / 2;
 ...
 public:
  ...
  void modify(const T& v, int x) { modify(v, x, 0, n_, 1); }
};
...
using SegmentTreeInfo = SegmentTreeT<Info>;
...
class SegmentTree {
  ...
  void rangeAdd(int L, int R, int64_t x, int l, int r, int p) {
    ...
    int m = (l + r) / 2;
    if (L < m) {
      rangeAdd(L, R, x, l, m, p << 1);
    }
    if (R > m) {
      rangeAdd(L, R, x, m, r, p << 1 | 1);
    }
    pull(p);
  }
...
class SegmentTreeAddCountMin {
  ...
  void pull(int p) { mx[p] = std::min(mx[p << 1], mx[p << 1 | 1]); }
...
// min/max version slightly hard: you should record min/max, and second min/max value:
// https://codeforces.com/gym/102992/problem/J
...
class PstSegTree {
  ...
  void pushUp(int p) { tree_[p].val = tree_[tree_[p].l].val + tree_[tree_[p].r].val; }
  ...
  int64_t query(int L, int R, int l, int r, int p) {
    ...
    int64_t ans = 0;
    if (L < m) {
      ans += query(L, R, l, m, tree_[p].l);
    }
    if (R > m) {
      ans += query(L, R, m, r, tree_[p].r);
    }
    return ans;
  }
 ...
 public:
  ...
  void update(int pos, int val, int p, int q) { update(pos, val, 0, n_, p, q); }
  // segment query, p is current version, q is new version
  int64_t query(int L, int R, int p) { return query(L, R, 0, n_, p); }
};
// https://www.luogu.com.cn/problem/P3919

// Fenwick inside a Persistent Segment Tree to get dynamic k-th smallest number(online)
class BitPstSegTree {
