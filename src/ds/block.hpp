...
class BlockAbs {
  ...
  std::vector<int> fa_;
  int find(int x) { return x == fa_[x] ? x : fa_[x] = find(fa_[x]); }
 ...
 public:
  ...
  int query(int x) { return find(x) * f_ - d_; }
};
...
class BlockMinus {
  ...
  void modifyAll(int x) {
    if (x < mx_ - delta_ - x) {
      for (int i = delta_ + 1; i <= x + delta_; ++i) {
        merge(i, i + x);
      }
      delta_ += x;
    } else {
      for (int i = mx_; i > x + delta_; --i) {
        merge(i, i - x);
      }
      mx_ = x + delta_;
    }
